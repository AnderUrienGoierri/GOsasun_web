...
require_once '../php_orri_includeak/hizkuntza_kargatu.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($email) && !empty($pasahitza)) {
        try {
            ...
            $stmt = $pdo->prepare("SELECT e.id AS erabiltzaile_id, e.email, e.pasahitza, e.rol_id, r.izena AS rol_izena, e.izena, e.abizenak 
                                   FROM Erabiltzaileak e 
                                   JOIN Rolak r ON e.rol_id = r.id 
                                   WHERE e.email = ? AND e.aktibo = 1");
            $stmt->execute([$email]);
